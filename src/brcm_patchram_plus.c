...
uchar hci_write_sleep_mode[] = { 0x01, 0x27, 0xfc, 0x0c,
	0x01, 0x02, 0x02, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00,
	0x00, 0x00 };
...
parse_baudrate(char *optarg)
{
	...
	if (validate_baudrate(baudrate, &d)) {
...
void
proc_read_chip_props()
{
	uchar hci_data[] = { 0x01, 0x0a, 0xfc, 0x05, 0x04, 0x51, 0xf0, 0x20, 0x00 };

	hci_send_cmd(hci_data, sizeof(hci_data));
...
void
proc_read_verbose_config()
{
	uchar hci_data[] = { 0x01, 0x79, 0xfc, 0x00 };
	hci_send_cmd(hci_data, sizeof(hci_data));
...
void
proc_read_local_version()
{
	uchar hci_data[] = { 0x01, 0x01, 0x10, 0x00 };
	hci_send_cmd(hci_data, sizeof(hci_data));
...
proc_baudrate()
{
	...
	cfsetospeed(&termios, d);
	cfsetispeed(&termios, d);
	tcsetattr(uart_fd, TCSANOW, &termios);
